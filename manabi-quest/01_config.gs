...
const LIMITS = {
  ...
  // 「ログ」シートをさかのぼる**日数**。
  //
  // 以前は行数（5000行 / 20000行）で切っていましたが、これには2つ問題がありました。
  //   1. 読んだあとに slice していたので、シートの読み込みは1行も減っていなかった
  //   2. 30人が毎日使うと「ログ」は1日750〜1000行たまるため、20000行は**1か月ぶん**しかない。
  //      CALENDAR_WEEKS（12週 ≒ 60登校日 ≒ 45000〜60000行）に足りず、
  //      がんばりカレンダー・連続日数がエラーも出さずに古いデータを取りこぼす
  // 日数で決めれば、1日あたりの行数が増えても取りこぼしません。
  //
  // CALENDAR_WEEKS × 7 = 84日より必ず大きくしてください。
...
/**
 * データベース（スプレッドシート）の容量に関するしきい値。
 *
 * スプレッドシートは **1ファイル 1,000万セル** までで、**空セルも数に入ります**。
 * シートは既定で 1000行 × 26列 のグリッドを持つため、4列しか使っていない「ログ」でも
 * 1行追記するたびに26セルを消費します（「未使用列のトリム」で詰められます）。
 *
 * 行数のしきい値は「読み込みが重くなりはじめる目安」で、上限ではありません。
 * 超えたら年度末アーカイブ（データを別ファイルへ退避し、行を削除）を検討します。
 */
const CAPACITY = {
  CELL_LIMIT: 10000000,        // スプレッドシート1ファイルあたりのセル数上限
  CELL_WARN: 0.6,              // これを超えたら注意（週次メールでお知らせします）
  CELL_ALERT: 0.8,             // これを超えたら早めのアーカイブを促します
  STUDY_LOG_WARN_ROWS: 20000,  // 「学習ログ」がこの行数を超えたらお知らせ
  LOG_WARN_ROWS: 100000        // 「ログ」がこの行数を超えたらお知らせ
};
